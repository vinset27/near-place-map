...
function hash32(input: string): number {
  // Simple, stable hash for UI placeholders (not crypto).
  let h = 2166136261;
...
export function placeholderImageDataUrl(input: {
  id: string;
  name: string;
  category?: Establishment["category"] | string | null;
}): string {
  ...
  const bg2 = pick(
    ["#111827", "#0b1220", "#052e16", "#3b0764", "#0f172a", "#1f2937", "#312e81", "#134e4a"],
    seed >>> 3,
  );
  // IMPORTANT: use full unicode codepoint (emoji-safe) to avoid producing lone surrogates.
